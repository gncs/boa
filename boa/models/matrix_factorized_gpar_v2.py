...
class MatrixFactorizedGPARModel(GPARModel):
    ...
    def __init__(self,
        ...
        for i in range(self.output_dim):
            ...
            out_length_scales = tf.Variable(tf.ones(i, dtype=tf.float64),
                                            name=f"{i}/output_length_scale", trainable=False)
    ...
    def initialize_hyperparameters(self,
        ...
        if length_scale_init == "median":
            ...
            # left length scale
            lls_init = ls_init + tf.random.uniform(shape=(self.output_dim, self.latent_dim),
                                                   dtype=tf.float64,
                                                   minval=-ls_rand_range,
                                                   maxval=ls_rand_range)

            # right length scale
            rls_init = ls_init + tf.random.uniform(shape=(self.latent_dim, self.input_dim),
                                                   dtype=tf.float64,
                                                   minval=-ls_rand_range,
                                                   maxval=ls_rand_range)
    ...
    def fit(self, xs, ys, optimizer_restarts=1) -> None:
        ...
        def negative_mf_gpar_log_likelihood(vs):
            ...
            length_scales = tf.matmul(vs[self.LLS_MAT],
                                      vs[self.RLS_MAT])

            for k in range(self.output_dim):
                # Create a single length scale vector by concatenating
                # the input and the output length scales
                gp_length_scales = tf.concat((length_scales[k, :],
                                              vs[f"{k}/output_length_scales"]),
                                             axis=0)
        ...
        while i < optimizer_restarts:
            ...
            if loss < best_loss:
                ...
                for j in range(self.output_dim):
                    ...
                    self.length_scales[j] = tf.concat((input_length_scales[j, :],
